...
5. Google Drive — EXACT OPERATIONS: evidence that the adopted runtime's Drive tool surface supports the six-step proof concretely — create, content read, update, verification read, `files.update(trashed=true)`, cleanup verification — under `drive.file` scope, with file IDs present in tool results (needed for artifact-id extraction). A partial or read-only Drive surface FAILS this gate. Plus the OAuth token provisioning path (informs D10).
